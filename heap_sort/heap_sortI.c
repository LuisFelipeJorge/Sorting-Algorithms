...
void heapSortI(long int n, long int a[]);//function that contains the sorting algorithm
...
void heapfy(long int p, long int m, long int a[]){ // function to create a heap data structure inside the input array
  // this time, we will not use recursion
  while (p < m) { // the parent node have always to be smaller then the number of element in our list od numbers
    ...
    if (f < m) { // the left child must be within the range of the input array
      comparisonsCounter++;

      if ((f+1) < m) { // the right child must respect the same condition
        comparisonsCounter++;
    ...
    p = f; // updating the value of p inside the 'while loop'
    // With this, the algorithm is acting as calling recursively the function for the next node
  }
}

//////////////////////////////////////////////////////////

void heapSortI(long int n, long int a[]) {
  ...
  // the algorithm looks like the recursive version, the difference is within the heapfy function
}
